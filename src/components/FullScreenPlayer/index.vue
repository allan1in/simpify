<template>
    <main @mousemove="isCursorMove = true" class="full-screen-container">
        <div class="full-screen-container__top">
            ...
            <button :class="{ 'full-screen-container__top__close-wrapper-show': isCursorMove || isCursorOverClose }"
        ...
        <div class="full-screen-container__content">
            <div class="full-screen-container__content__cover-wrapper"
                :class="{ 'full-screen-container__content__cover-wrapper-small': !isCursorMove }" v-if="album.images">
                <img class="full-screen-container__content__cover-wrapper__cover" :src="album.images[0].url"
                    :alt="track">
            </div>
            <div class="full-screen-container__content__text-wrapper">
                <h1 class="full-screen-container__content__text-wrapper__name" v-if="track">{{ track.name }}</h1>
                <h2 class="full-screen-container__content__text-wrapper__artist"><span
                        v-for="(artist, index) in artists" :key="artist.uri" v-if="artists.length">{{ (index === 0 ? ''
                            : ', ') + artist.name }}</span></h2>
            </div>
        </div>
...
export default {
    ...
    computed: {
        ...mapWritableState(usePlayerStore, ['album', 'track', 'artists', 'isPause', 'repeatMode', 'isShuffle', 'isMute', 'volume', 'showFullScreenPlayer']),
    },
    components: {
        IconPrimaryLogo,
        IconClose,
    ...
    methods: {
        ...
        ...mapActions(usePlayerStore, ['togglePlay', 'nextTrack', 'preTrack', 'setRepeatMode', 'toggleShuffle'])
    },
    watch: {
        isCursorMove: {
